#!/bin/zsh
# 在桌面生成 zip：内含独立 .app，对方无需安装 Python。
# 支持模式（通过 PIXELFORGE_PACKAGE_TARGET 控制）：
#   native      仅当前打包机架构（默认）
#   arm64       Apple Silicon 专用（需在 M 芯片 Mac 上执行）
#   universal2  Intel + Apple Silicon 通用（需 universal2 Python/依赖）
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

HOST_ARCH=$(uname -m)
TARGET="${PIXELFORGE_PACKAGE_TARGET:-native}"
TARGET="${TARGET:l}"

ARCH="$HOST_ARCH"
export PIXELFORGE_MAC_ARCH=native

case "$TARGET" in
  native|current|host|"")
    ARCH="$HOST_ARCH"
    export PIXELFORGE_MAC_ARCH=native
    ;;
  arm64|apple|m|mchip)
    if [[ "$HOST_ARCH" != "arm64" ]]; then
      echo "错误：arm64 包需要在 Apple Silicon (arm64) Mac 上构建。当前为 $HOST_ARCH。"
      echo "建议：在 M 芯片 Mac 上执行本脚本，或改用 universal2 模式。"
      exit 1
    fi
    ARCH="arm64"
    export PIXELFORGE_MAC_ARCH=native
    ;;
  universal2|universal|fat)
    ARCH="universal2"
    export PIXELFORGE_MAC_ARCH=universal2
    ;;
  *)
    echo "错误：未知 PIXELFORGE_PACKAGE_TARGET=$TARGET（可选：native / arm64 / universal2）"
    exit 1
    ;;
esac

STAMP=$(date +%Y%m%d)
OUT_NAME="《图片压缩与格式转化》-免Python-${ARCH}-${STAMP}.zip"
OUT_PATH="${HOME}/Desktop/${OUT_NAME}"

# 国内 pip 慢可取消下一行注释
# export PIP_INDEX_URL="https://pypi.tuna.tsinghua.edu.cn/simple"

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ① 准备打包专用环境（首次约 2～5 分钟）"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

if ! command -v python3 &>/dev/null; then
  echo "错误：未找到 python3，请先安装 Python。"
  exit 1
fi

if [[ ! -d ".venv-pack" ]]; then
  python3 -m venv .venv-pack
fi
# shellcheck disable=SC1091
source ".venv-pack/bin/activate"
pip install -q -U pip
pip install -q -r requirements-web.txt -r requirements-build.txt

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ② 构建独立应用 PixelForge Studio.app（约 1～2 分钟）"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

python -m PyInstaller --noconfirm PixelForge.spec

APP_SRC="$ROOT/dist/PixelForge Studio.app"
if [[ ! -d "$APP_SRC" ]]; then
  echo "错误：未找到构建产物 $APP_SRC"
  exit 1
fi

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ③ 写入 zip"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

TDIR=$(mktemp -d)
trap 'rm -rf "$TDIR"' EXIT

mkdir -p "$TDIR/PixelForge"
rsync -a \
  --exclude '.git' \
  --exclude '.venv' \
  --exclude '.venv-share' \
  --exclude '.venv-universal' \
  --exclude '.venv-pack' \
  --exclude '.wheelcheck' \
  --exclude 'build' \
  --exclude 'dist' \
  --exclude '__pycache__' \
  --exclude '*.pyc' \
  --exclude '.DS_Store' \
  --exclude '《图片压缩与格式转化》-免Python-*.zip' \
  "$ROOT/" "$TDIR/PixelForge/"

# 用构建好的 app 覆盖 rsync 可能带来的旧 dist 副本（rsync 已排除 dist）
rm -rf "$TDIR/PixelForge/PixelForge Studio.app"
cp -R "$APP_SRC" "$TDIR/PixelForge/"

python3 - <<PY
from pathlib import Path
import zipfile

src_root = Path("$TDIR") / "PixelForge"
out_path = Path("$OUT_PATH")
with zipfile.ZipFile(out_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
    for path in src_root.rglob("*"):
        arcname = path.relative_to(Path("$TDIR"))
        if path.is_dir():
            zf.writestr(str(arcname).replace("\\\\", "/") + "/", b"")
        else:
            zf.write(path, str(arcname).replace("\\\\", "/"))
PY

echo ""
echo "已生成：$OUT_PATH"
if [[ "$ARCH" == "universal2" ]]; then
  echo "（内含独立应用，支持 Intel + Apple Silicon）"
else
  echo "（内含独立应用，对方 Mac 需为 ${ARCH} 架构）"
fi
echo ""
echo "使用步骤：解压 → 先读「使用说明.txt」→ 双击 PixelForge Studio.app"
osascript -e "display notification \"已保存到桌面（免 Python）\" with title \"PixelForge 分享包\"" 2>/dev/null || true
